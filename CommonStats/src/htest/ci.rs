...
///
/// Unlike [`ci_mean_diff`], this inverts to the Welch two-sample p-value: SE =
/// √(vA/nA + vB/nB) and df from Welch–Satterthwaite, matching `t_test_two(…,
/// Welch)`. `a`, `b`: observations, NaN dropped under Omit, each needs ≥ 2 finite
/// values; `level`: coverage in (0, 1). [`StatError::ProbabilityOutOfRange`] /
/// [`StatError::TooFewObservations`].
