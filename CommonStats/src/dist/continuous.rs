...
/// (ported from mcpower `t_ppf`). Matches `scipy.stats.t(df)`.
#[derive(Debug, Clone, Copy, PartialEq)]
...
impl ContinuousCdf for StudentT {
    ...
    fn quantile(&self, p: f64) -> Result<f64, StatError> {
        ...
        // Initial guess (Cornish–Fisher / closed forms), then Newton.
...
/// Newton on the gamma CDF (ported from mcpower `chi2_ppf`).
...
/// Quantile = Wilson–Hilferty seed + Newton (ported from mcpower `f_ppf`).
...
impl ContinuousCdf for Exponential {
    fn cdf(&self, x: f64) -> f64 {
        if x <= 0.0 { 0.0 } else { -(-self.rate * x).exp_m1() } // 1 - e^{-λx}
    }
    fn sf(&self, x: f64) -> f64 {
...
impl ContinuousCdf for Weibull {
    fn cdf(&self, x: f64) -> f64 {
        if x <= 0.0 { return 0.0; }
        -(-(x / self.scale).powf(self.shape)).exp_m1() // 1 - e^{-(x/λ)^k}
    }
    fn sf(&self, x: f64) -> f64 {
        if x <= 0.0 { return 1.0; }
...
impl ContinuousCdf for Gamma {
    ...
    fn quantile(&self, p: f64) -> Result<f64, StatError> {
        ...
        if p == 1.0 { return Ok(f64::INFINITY); }
        let a = self.shape;
        // Seed: χ²(2α) Wilson–Hilferty, then x = chi2/(2β). (chi2 has df k=2α.)
        let k = 2.0 * a;
        let z = norm_quantile(p);
        let h = 2.0 / (9.0 * k);
